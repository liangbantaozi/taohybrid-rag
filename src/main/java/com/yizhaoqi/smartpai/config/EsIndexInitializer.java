...
public class EsIndexInitializer implements CommandLineRunner {
    ...
    private String knowledgeIndexName;

    @Override
    public void run(String... args) throws Exception {
        try {
            logger.info("正在初始化索引 '{}'... endpoint={}://{}:{}, username={}",
                    knowledgeIndexName, scheme, host, port, maskUsername(username));
            initializeIndex();
    ...
    private void initializeIndex() throws Exception {
        // 检查索引是否存在
        BooleanResponse existsResponse = esClient.indices().exists(ExistsRequest.of(e -> e.index(knowledgeIndexName)));
        if (!existsResponse.value()) {
            createIndex();
        } else {
            logger.info("索引 '{}' 已存在", knowledgeIndexName);
        }
    }
    ...
    private void createIndex() throws Exception {
        ...
        CreateIndexRequest createIndexRequest = CreateIndexRequest.of(c -> c
                .index(knowledgeIndexName) // 索引名称
                .withJson(new StringReader(mappingJson)) // 使用 JSON 文件定义映射
        );
        esClient.indices().create(createIndexRequest);
        logger.info("索引 '{}' 已创建", knowledgeIndexName);
    }
