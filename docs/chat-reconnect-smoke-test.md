...
- logs in with `admin`; set `CHAT_SMOKE_USERNAME` to override it
- reads the password from `CHAT_SMOKE_PASSWORD` or `--password`
...
```bash
CHAT_SMOKE_PASSWORD=your-local-test-password \
node scripts/chat-reconnect-smoke-test.mjs \
